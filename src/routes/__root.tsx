...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      ...
      { name: "author", content: "Sunrise Assurance" },
...
const NAV = [
  { to: "/", label: "Claims assistant" },
  { to: "/inbox", label: "Escalation inbox" },
  { to: "/decision-log", label: "Decision log" },
] as const;

function SiteHeader() {
  return (
    <header className="sticky top-0 z-30 border-b border-border bg-parchment/95 backdrop-blur">
      <div className="mx-auto flex h-14 max-w-[1600px] items-center gap-6 px-4 sm:px-6">
        <Link to="/" className="flex items-baseline gap-2.5">
          ...
          <span className="hidden border-l border-rule pl-2.5 text-[11px] tracking-wide text-muted-foreground sm:inline">
            Claims Initiation
          </span>
        </Link>
        ...
        <nav className="ml-auto flex items-center gap-1">
          {NAV.map((item) => (
            <Link
              key={item.to}
              to={item.to}
              className="rounded px-2.5 py-1.5 text-[13px] text-muted-foreground transition-colors hover:bg-secondary hover:text-foreground"
              activeProps={{
                className:
                  "rounded px-2.5 py-1.5 text-[13px] font-medium text-foreground bg-secondary",
              }}
